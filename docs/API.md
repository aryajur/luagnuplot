...
- [luacmd Terminal Functions](#wxlua-terminal-functions)
...
Retrieve the list of drawing commands generated by gnuplot when using the luacmd terminal.
...
gnuplot.cmd("set grid")
gnuplot.cmd("plot 'tempdata.txt' with lines title 'y = x² - 2x + 1'")
...
local result = gnuplot.get_commands()
if not result then
  print("No commands captured - did you use luacmd terminal?")
end
