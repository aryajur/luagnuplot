...
-- Analyze commands to get coordinate ranges (for debugging)
local function analyze_command_coords(commands)
    local min_x, max_x = math.huge, -math.huge
    local min_y, max_y = math.huge, -math.huge
    local coord_count = 0

    for _, cmd in ipairs(commands) do
        if cmd.type == CMD_MOVE or cmd.type == CMD_VECTOR or
           cmd.type == CMD_TEXT or cmd.type == CMD_POINT then
            if cmd.x then
                min_x = math.min(min_x, cmd.x)
                max_x = math.max(max_x, cmd.x)
                coord_count = coord_count + 1
            end
            if cmd.y then
                min_y = math.min(min_y, cmd.y)
                max_y = math.max(max_y, cmd.y)
            end
        end
        if cmd.type == CMD_VECTOR and cmd.x2 and cmd.y2 then
            min_x = math.min(min_x, cmd.x2)
            max_x = math.max(max_x, cmd.x2)
            min_y = math.min(min_y, cmd.y2)
            max_y = math.max(max_y, cmd.y2)
        end
    end

    return {
        min_x = min_x == math.huge and 0 or min_x,
        max_x = max_x == -math.huge and 0 or max_x,
        min_y = min_y == math.huge and 0 or min_y,
        max_y = max_y == -math.huge and 0 or max_y,
...
function wxgnuplot.new(parent, id, pos, size)
    ...
    plot.panel:Connect(wx.wxEVT_SIZE, function(event)
        ...
        print(string.format("[RESIZE] Client size: %dx%d, Current plot size: %dx%d",
              new_width, new_height, plot.width, plot.height))
        ...
        if new_width > 0 and new_height > 0 and
           (new_width ~= plot.width or new_height ~= plot.height) then
            print(string.format("[RESIZE] Size changed! Updating from %dx%d to %dx%d",
            ...
            if #plot.commands > 0 then
                print(string.format("[RESIZE] Re-executing %d commands", #plot.commands))
                plot:execute()
            else
                print("[RESIZE] No commands to execute")
            end
        else
    ...
    function plot:execute()
        print(string.format("[EXECUTE] Starting with target size: %dx%d", self.width, self.height))
        ...
        if not self.gnuplot_initialized then
            print("[EXECUTE] Initializing gnuplot")
            self.gnuplot.init()
        ...
        for i, cmd in ipairs(self.commands) do
            ...
            if cmd_lower == "plot" or cmd_lower == "splot" or cmd_lower == "replot" then
                ...
                local term_cmd = string.format("set terminal luacmd size %d,%d", self.width, self.height)
                print(string.format("[EXECUTE] Injecting before plot: %s", term_cmd))
                self.gnuplot.cmd(term_cmd)
            end

            -- Execute the user command
            print(string.format("[EXECUTE] Command %d: %s", i, cmd))
        ...
        if not result or not result.commands then
            print("[EXECUTE] ERROR: Failed to get gnuplot commands")
            return false, "Failed to get gnuplot commands"
        end

        print(string.format("[EXECUTE] Got result: %d commands, size from gnuplot: %dx%d",
              #result.commands, result.width or 0, result.height or 0))

        -- Analyze coordinate ranges in commands
        local coords = analyze_command_coords(result.commands)
        print(string.format("[EXECUTE] Command coordinate ranges: X[%d to %d], Y[%d to %d] (%d coords)",
              coords.min_x, coords.max_x, coords.min_y, coords.max_y, coords.coord_count))

        -- Render to bitmap at the size we requested
        print(string.format("[EXECUTE] Rendering bitmap at: %dx%d", self.width, self.height))
        self.bitmap = render_commands(result.commands, self.width, self.height)

        if not self.bitmap then
            print("[EXECUTE] ERROR: Failed to render bitmap")
            return false, "Failed to render bitmap"
        end

        print(string.format("[EXECUTE] Bitmap created: %dx%d",
              self.bitmap:GetWidth(), self.bitmap:GetHeight()))

        -- Refresh panel
        self.panel:Refresh()
        print("[EXECUTE] Panel refreshed")
